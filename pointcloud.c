...
PointCloud loadPointcloud(const char *path) {
    ...
    normalizePointcloudScale(cloud, 1.0);
    return cloud;
}
...
void normalizePointcloudScale(PointCloud cloud, float targetRadius) {
    ...
    for(size_t point = 0; point < cloud.count; point++){
        Vector3 pos = cloud.points[point].position;
        float radius = sqrtf(pos.x * pos.x + pos.y * pos.y + pos.z * pos.z);
    ...
    float factor = targetRadius / maxRadius;

    for(size_t point = 0; point < cloud.count; point++){
        cloud.points[point].position.x *= factor;
        cloud.points[point].position.y *= factor;
        cloud.points[point].position.z *= factor;
    }
}
